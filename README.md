...
> **⚠️ 重要提示**
>
> Cloudflare Workers 默认提供的 `*.workers.dev` 域名在中国大陆地区已被屏蔽，无法直接访问。
>
> **解决方案：**
> - ✅ **强烈建议绑定自定义域名**（参见 [绑定自定义域名](#绑定自定义域名) 章节）
> - ✅ 自定义域名可以正常在国内使用
> - ✅ 绑定域名完全免费，只需在 Cloudflare 托管您的域名
...
4. 按照提示修改域名的 DNS 服务器指向 Cloudflare

#### 步骤 2：配置 Worker 路由

1. 在 Cloudflare Dashboard 中，选择您的域名
2. 进入 "Workers 路由" 或 "Workers Routes"
3. 点击 "添加路由" 或 "Add Route"
4. 填写以下信息：
   - **路由**：`proxy.example.com/*` （替换为您的子域名）
   - **Worker**：选择 `sh-proxy`
5. 点击保存

#### 步骤 3：添加 DNS 记录（可选）

如果上述方法不生效，可能需要手动添加 DNS 记录：
